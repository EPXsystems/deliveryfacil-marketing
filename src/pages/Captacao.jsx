...
export default function Captacao() {
  ...
  async function handleSearch(e) {
    ...
    try {
      const res = await fetch(`${SCRAPER_URL}/scrape/maps`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ categoria: categoria.trim(), cidade: cidade.trim(), quantidade }),
      })
  ...
  function handleAddToPipeline(lead) {
    // Salva no localStorage para o Pipeline consumir
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="px-6 py-5 border-b border-[#1f1f1f] flex-shrink-0">
        <form onSubmit={handleSearch} className="flex flex-wrap gap-3 items-end">
          ...
          <button
            type="submit"
            disabled={loading || !categoria.trim() || !cidade.trim()}
      ...
      <div className="flex-1 overflow-auto px-6 py-4">
        ...
        {/* Error */}
        {error && !loading && (
          <div className="flex items-center gap-3 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3 mt-2">
        ...
        {!loading && !error && leads.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <div className="text-center">
              ...
              <p className="text-[#444] text-xs mt-1">Preencha os campos e clique em Buscar Leads</p>
            </div>
          </div>
        )}
          ...
          <>
            <div className="flex items-center justify-between mb-4">
              <p className="text-[#666] text-sm">
                <span className="text-white font-semibold">{leads.length}</span> leads encontrados
                {' '}para <span className="text-[#FF4D1C]">{categoria}</span> em <span className="text-[#FF4D1C]">{cidade}</span>
              </p>
            </div>
